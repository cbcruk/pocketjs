...
//! framebuffer. The target contract is portrait 379×512 @2x, but `/dev/fb0`
...
/// xres/yres. Kobo's mxc_epdc driver reports the orientation nickel last set
/// rather than the one this host needs, so that value must remain informational.
...
mod tests {
    ...
    fn reported_rotation_is_used_only_when_visible_dimensions_agree() {
        assert_eq!(
            compatible_reported_rotation(3, 758, 1024, 758, 1024).unwrap(),
            None
        );
        assert_eq!(
            compatible_reported_rotation(3, 758, 1024, 1024, 758).unwrap(),
            Some(Rotation::R270)
        );
        assert_eq!(
            compatible_reported_rotation(2, 758, 1024, 758, 1024).unwrap(),
            Some(Rotation::R180)
        );
        assert!(compatible_reported_rotation(9, 758, 1024, 758, 1024).is_err());
    }
