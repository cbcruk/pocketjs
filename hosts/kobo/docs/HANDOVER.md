...
- **nickel을 없애면 nickel이 읽던 파이프도 없어진다.** rcS가 만드는
  ...
  **네트워크는 멀쩡하고, 새는 건 프로세스뿐**이다. 런처가 독자 역할을 넘겨받되
  ...
  ```
- **이 개체의 배터리는 부풀어 있다 (2026-09-09 확인).** 게이지가 100%를 가리켜도
  케이블을 뽑으면 곧바로 꺼진다 — 셀이 부하를 못 버틴다. **교체 전까지 꽂아두고
  쓴다.** 부푼 리튬 셀은 자리를 비우는 동안 충전하지 않고, 누르거나 구부리지
  않는다. 2012년 기기이므로 14년 된 셀이고 예상 범위의 수명 종료다.
  `battery.log`의 교체 이전 구간은 방전 곡선이 아니니 그렇게 읽지 말 것.
- **이 모델에는 사용자가 부를 수 있는 절전 상태가 없다.** 이유가 두 개고 각각
  독립적이다. (1) 커널이 드라이버 서스펜드에서 멈춘다 — `pm_test`로 `freezer`는
  통과, `devices`에서 정지. 호스트를 SIGSTOP으로 재우고 Wi-Fi를 내려도 같으니
  우리 탓이 아니다. (2) 깨울 방법이 없다.
- **전원 버튼은 소프트웨어에 보이지 않는다.** `/proc/interrupts`에서 `mxckpd`는
  부팅 이래 0이고 별도의 `power_key` IRQ도 부팅 때 1회뿐이며, 동작 중에 눌러도
  **양쪽 다 증가하지 않는다.** PMIC가 하드웨어에서 먹고, 커널은 서스펜드용 wakeup
  소스로만 안다. `mxckpd`가 KEY_POWER를 **선언**하기 때문에 호스트가 event0을
  전원 키로 고르지만, 그 노드는 영원히 조용하다 — zForce 축 범위와 같은 함정이다.
  **그러므로 0009의 전원 키 처리는 이 기기에서 동작한 적이 없고, "길게 눌러 Kobo
  UI로 돌아간다"는 탈출구도 존재하지 않는다.** 사용자가 본 "길게 누르면 꺼짐"은
  하드웨어 전원 차단이다.
- **`rtc`에 `wakealarm`이 없다.** 타이머 복귀도 없다는 뜻이라, 설령 서스펜드가
  됐더라도 돌아올 길이 없었다.
