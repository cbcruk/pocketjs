...
/// framebuffer, fusing the grayscale conversion into the rasterizer so an
/// e-ink host never allocates an intermediate RGBA8 surface. Channels use the
...
/// other targets, so a converted RGBA8 render and a native Gray8 render agree
/// pixel-for-pixel.
pub fn render_scaled_gray8(ui: &impl RenderResources, words: &[u32], fb: &mut [u8], scale: u32) {
...
/// persistent framebuffer. The signature keeps a Gray8 tracker from being
/// reused against a differently formatted surface.
